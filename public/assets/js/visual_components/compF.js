function drawFComponent(svg, table, componentName, componentType, redrawCallback) {

	const {mxValue, mnValue} = calcMaxMinFromTable(table);
	...
	// draw main start
		function yAxisFormat(num) {
			return numberDisplay(num * 100, 'pts', true);
		}
		drawAxis(
			svgM, 
			xScale, 
		...
		for (var i = table[0].length - 1; i > 0 ; i --) {
			var lastVal = table[table.length-1][i];

	  		drawText(
	  			svgM, xScale(table.length-1), (yScale(lastVal)+5), 
	  			0, colors[i-1], numberDisplay(lastVal*100, 'pts'));
		}
	...
	// draw footer start
		...
		for (var i = 1; i < table[0].length; i ++) {
			...
			drawOneLineDotDescription(svgF, xScaleFooter(xITable[(i-1)%5+1]), parseInt((i-1)/5)*10, colors[i-1], (i==1), table[0][i]);
		}
	// draw footer end
